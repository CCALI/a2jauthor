...
TGuide.prototype.HotDocsAnswerSetVariable = function (variable) {
  ...
  } else {	// Use the field type to determine the HotDocs type to use.
  ...
  function getXMLValue (value) {
    ...
    var xmlV
    if (typeof value === 'undefined' || value === null || value === '') {	// 2014-06-02 SJG Blank value for Repeating variables MUST be in answer file (acting as placeholders.)
      xmlV = '<' + ansType + ' UNANS="true">' + '' + '</' + ansType + '>'
    } else {
      var val = escapeHtml(value)
      if (varType === CONST.vtMC) {	// 2015-01-12 MCValue > SelValue
  ...
  var vi
  var xml = ''
  if (variable.repeating === true) {	// Repeating variables are nested in RptValue tag.
    ...
    if (!(typeof value === 'undefined' || value === null || value === '')) {	// 2014-06-02 SJG Blank value for non-repeating must NOT be in the answer file.
...
TGuide.prototype.updateVarsForAnswerFile = function () {	// 6/30/10 Update bookmark, history info just before saving the answer file.
