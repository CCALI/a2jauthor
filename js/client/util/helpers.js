define(['jquery', 'can', 'can/view/stache', 'can/route'], function($, can) {

	can.stache.registerHelper('~', function(prop, options) {
		return options.context[prop];
	});

	can.stache.registerHelper('inc', function(val, options) {
	...
	can.stache.registerHelper('imagePath', function(prefix, src, options) {
	...
	can.stache.registerHelper('prefix', function(prefix, str, options) {
		prefix = typeof prefix === 'function' ? prefix() : prefix;
