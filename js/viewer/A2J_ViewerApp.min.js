...
return!1},_mouseUp:function(b){return a.ui.mouse.prototype._mouseUp.call(this,b)},_moveItem:function(a,c,d){var e=parseFloat(a.css("left")),f=parseFloat(a.css("top"));isNaN(e)&&(e=a.offset().left);isNaN(f)&&(f=a.offset().top);c*=1/this.z;d*=1/this.z;a.css({left:e-c,top:f-d})},changeScale:function(a){this.z=a;a="scale("+this.z+")";console.log(a);this.element.css({"-webkit-transform":a,"-moz-transform":a,"-ms-transform":a,"-o-transform":a,transform:a})}})})(jQuery);(function(a){a.widget("ui.combobox",{_create:function(){var b,c=this,d=!1,e=this.element.hide(),f=e.children(":selected"),f=f.val()?f.text():"",g=this.wrapper=a("<span>").addClass("ui-combobox").insertAfter(e);b=a("<input>").appendTo(g).val(f).attr("title","").addClass("ui-state-default ui-combobox-input").autocomplete({delay:0,minLength:0,source:function(b,c){var d=RegExp(a.ui.autocomplete.escapeRegex(b.term),"i");c(e.children("option").map(function(){var c=a(this).text();if(this.value&&(!b.term||
d.test(c)))return{label:c.replace(RegExp("(?![^&;]+;)(?!<[^<>]*)("+a.ui.autocomplete.escapeRegex(b.term)+")(?![^<>]*>)(?![^&;]+;)","gi"),"<strong>$1</strong>"),value:c,option:this}}))},select:function(a,b){b.item.option.selected=!0;c._trigger("selected",a,{item:b.item.option})},change:function(c,d){if(!d.item){var f=a(this).val(),g=RegExp("^"+a.ui.autocomplete.escapeRegex(f)+"$","i"),l=!1;e.children("option").each(function(){if(a(this).text().match(g))return this.selected=l=!0,!1});l||(a(this).val("").attr("title",
f+" didn't match any item").tooltip("open"),e.val(""),setTimeout(function(){b.tooltip("close").attr("title","")},2500),b.data("ui-autocomplete").term="")}}}).addClass("ui-widget ui-widget-content ui-corner-left");b.data("ui-autocomplete")._renderItem=function(b,c){return a("<li>").append("<a>"+c.label+"</a>").appendTo(b)};a("<a>").attr("tabIndex",-1).attr("title","Show All Items").tooltip().appendTo(g).button({icons:{primary:"ui-icon-triangle-1-s"},text:!1}).removeClass("ui-corner-all").addClass("ui-corner-right ui-combobox-toggle").mousedown(function(){d=
b.autocomplete("widget").is(":visible")}).click(function(){b.focus();d||b.autocomplete("search","")});b.tooltip({tooltipClass:"ui-state-highlight"})},_destroy:function(){this.wrapper.remove();this.element.show()}})})(jQuery);var CONST={devShowTesting:!1,showXML:0,uploadURL:"",uploadGuideURL:"",AJAXLoader:'<span class="loader">&nbsp;</span>"',A2JVersionNum:"5.0.1.15",A2JVersionDate:"2014-04-17",CAVersionNum:"5.0.0",CAVersionDate:"2013-04-15",navigationVarName:"A2J Navigation TF",interviewIncompleteVarName:"A2J Interview Incomplete TF",bookmarkVarName:"A2J Bookmark",historyVarName:"A2J History",interviewIDVarName:"A2J Interview ID",ptPopup:"Popup",ftButton:"button",ftText:"text",ftTextLong:"textlong",ftTextPick:"textpick",
...
FieldPrompts_radio:"계속하기 전에 강조 표시된 선택란에서 응답을 선택해야 합니다.",FieldPrompts_checkbox:"계속하려면 1개 이상의 체크박스를 선택해야 합니다.",FieldPrompts_checkboxNOTA:'계속하려면 2개 이상의 체크박스를 선택하거나 "위 내용 중 해당 사항 없음"을 선택하십시오.',Ordinals1:"첫째",Ordinals2:"둘째",Ordinals3:"셋째",Ordinals4:"넷째",Ordinals5:"다섯째",Ordinals6:"여섯째",Ordinals7:"일곱째",Ordinals8:"여덟째",Ordinals9:"아홉째",Ordinals10:"열 번째",Ordinals11:"열한 번째",Ordinals12:"열두 번째",Ordinals13:"열세 번째"};
...
dayNames:"일월화수목금토".split(""),dayNamesShort:"일월화수목금토".split(""),dayNamesMin:"일월화수목금토".split(""),weekHeader:"Wk",dateFormat:"yy-mm-dd",firstDay:0,isRTL:!1,showMonthAfterYear:!1,yearSuffix:"년"}}));function fixPath(a){if(0===a.indexOf("http"))return a;var b=gGuidePath+urlSplit(a).file;trace("fixPath",a,b);return b}function loadXMLList(a){var b=$(a.data);$(b,"option:first").prepend('<option value="">Choose from this list</option>');$(a.elt).html(b.html()).val(a.val)}
...
changeYear:!0};isBlankOrNull(g.min)||(trace("minDate",g.min),j.minDate=g.min);isBlankOrNull(g.max)||(trace("maxDate",g.max),j.maxDate=g.max);$.datepicker.setDefaults($.datepicker.regional[gGuide.language]);j.dateFormat="mm/dd/yyyy";l.datepicker(j);break;case CONST.ftGender:l=h+"M";h+="F";k=gGuide.goodGender(k);l=$("<div/>").append($('<input type=radio id="'+l+'" name="'+j+'"></input>').prop("checked","M"===k)).append($("<label/>").attr("for",l).html(lang.Male)).append("<br/>").append($('<input type=radio id="'+
h+'" name="'+j+'"></input>').prop("checked","F"===k)).append($("<label/>").attr("for",h).html(lang.Female));break;case CONST.ftRadioButton:n=$('<input type=radio id="'+h+'" name="'+j+'"></input>').prop("checked",k===g.value);break;case CONST.ftCheckBox:n=$('<input type=checkbox id="'+h+'"  name="'+j+'"></input>').prop("checked",k);break;case CONST.ftCheckBoxNOTA:n=$('<input type=checkbox id="'+h+'"  name="'+j+'"></input>')}j=$('<div class="field"/>');g.required&&j.addClass("required");null!==n?j.append($('<div class="labelinput"/>').append([n,
...
236,197,24]],signs:[[619,438,1],[550,200,0.6]]}],e=1>=c.length-a?e[0]:e[1],f=b(e.ground,0,0)+b("A2J5_CourtHouse.png",e.ch[0],e.ch[1],e.ch[2],e.ch[3]),g;for(g=0;g<e.steps.length;g++)var h=a+g,j=e.steps[g],k=0===h?0:(h-1)%4+1,f=f+b("step_circle_0.png",j[0],j[1],j[2],j[3]),j=e.signs[g],m=j[2],m="-moz-transform: scale("+m+");",f=f+('<div class="stepsign" style="position:absolute; left:'+j[0]+"px; top: "+j[1]+"px;"+m+'"><div class="stepsigncolor Step'+k+'" ></div>\t<div class="stepnumber" >'+c[h].number+
...
function gotoPageView(a){window.setTimeout(function(){var b=gGuide.pages[a];"undefined"===typeof b?(dialogAlert("Page is missing: "+a),traceLogic("Page is missing: "+traceTag("page",a))):(gPage=b,A2JViewer.layoutPage($("#page-viewer .A2JViewer"),gPage))},1)}function main(){loadGuideFile(gStartArgs.templateURL,"")}window.addEventListener("message",receiveMessage,!1);function receiveMessage(a){gStartArgs=a.data;main()};
