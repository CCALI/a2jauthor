...
var A2JViewer = {
  ...
  refreshVariables: function() {
    ...
    // List of variables *including* array element values such as
    // child name#1, child name#2, etc.
    var vn;
    for (vn in g.vars) {
      if (g.vars.hasOwnProperty(vn)) {
        ...
        if (v.repeating) {
          for (var i = 1; i < v.values.length; i++) {
            sortvars.push({v: v, i: i, name: v.name + '#' + i});
          }
        } else {
          sortvars.push({v: v, i: 1, name: v.name});
        }
      }
    }
  ...
  layoutPage: function(div, page) {
    ...
    if (div.html() === '') {
      ...
      $('.alertCounter').hide().click(function() {
