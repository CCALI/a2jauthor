...
describe('<a2j-viewer-sign-text> ViewModel', () => {
  ...
  it('displayText', () => {
    ...
    assert.equal(vm.attr('displayText'), 'slightly longer text with a space as the 51st char...', 'should truncate to 50 chars and add an ellipsis');

    vm.attr('text', 'long text with a space as the 50th character in the middle of a word');
    assert.equal(vm.attr('displayText'), 'long text with a space as the 50th character in...', 'should truncate to last full word before 50th char and add an ellipsis')
