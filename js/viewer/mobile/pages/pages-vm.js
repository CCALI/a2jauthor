...
export default CanMap.extend('PagesVM', {
  ...
  navigate (button, el, ev) {
    ...
    if (anyFieldHasError) {
      ...
      if (rState.previewActive && vm.hasSpecialButton(button)) {
        ev && ev.preventDefault()
        vm.previewActiveResponses(button, ev)
      }

      // RESUME button skips rest of navigate
      ...
      if (button.next === constants.qIDFAIL) {
        vm.handleFailButton(button, vm)
        return
      }

      // Set answers for buttons with values
      if (button.name) {
        vm.handleButtonValue(button, vm, page, logic)
      }

      // handle afterLogic
      vm.handleCodeAfter(button, page, logic, vm)

      // Don't post to the server in Author Preview aka previewActive
      if (!rState.previewActive && (vm.shouldPostOrAssemble(button))) {
        vm.handleServerPost(rState, button, vm, ev)
      }

      // user has selected to navigate to a prior question
      if (button.next === constants.qIDBACK) {
        vm.handleBackButton(rState, button, logic)
      }

      vm.handleGotoOrNormalNav(rState, button, logic, vm)
  ...
  handleGotoOrNormalNav (rState, button, logic, vm) {
  ...
  handleButtonValue (button, vm, page, logic) {
  ...
  handleCodeAfter (button, page, logic, vm) {
  ...
  handleServerPost (rState, button, vm, ev) {
  ...
  handleBackButton (rState, button, logic) {
