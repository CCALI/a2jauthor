(function (root, factory) {
  ...
  return function (gGuide,
    ...
    TLogic.prototype._CAJA = function (c) {
      window.event.trigger(window, 'traceLogic', { '_CAJA': { format: 'code', msg: c } }, false)
		};
    TLogic.prototype._IF = function (d, c, e) {
      if ((e) === true) {
        window.event.trigger(window, 'traceLogic', {
    ...
    TLogic.prototype._VS = function (c, varname, varidx, val) {
      window.event.trigger(window, 'traceLogic', { '_VS': { msg: c } }, false)
    ...
    TLogic.prototype._GO = function (c, pageName) {
      this.GOTOPAGE = pageName
			window.event.trigger(window, 'traceLogic', { '_GO': { msg: c } }, false)
		};
    TLogic.prototype._TRACE = function (c, exp) {
      window.event.trigger(window, 'traceLogic', { '_TRACE-c': { msg: c } }, false)
			this.indent++
			window.event.trigger(window, 'traceLogic', { '_TRACE-exp': { format: 'info', msg: exp } }, false)
			this.indent--
		};
        ...
        try {
          ...
          var message = {}
            ...
            msg: 'executeScript.error: ' + e.lineNumber + ': ' + e.message
					...
					window.event.trigger(window, 'traceLogic', message, false)
