...
function pageNameFieldsForTextTab (pagefs, page) {
  ...
  if (page.type !== window.CONST.ptPopup) {
    ...
    var f
    var labelChangeFnc = function (val, field) {
      field.label = val
    }
    var defValueChangeFnc = function (val, field) {
      field.value = val
    }
    var invalidChangeFnc = function (val, field) {
      field.invalidPrompt = val
    }
    for (f in page.fields) {
      ...
      if (window.gPrefs.showText === 2 || field.invalidPrompt !== '') {
        ff.append(
          window.form.htmlarea({
            label: 'Custom Invalid Prompt:',
            value: field.invalidPrompt,
            field: field,
            change: invalidChangeFnc
          })
        )
      }
      pagefs.append(ff)
    ...
    var bi
    var btnLabelChangeFnc = function (val, b) {
      b.label = val
    }
    var bntDevValChangeFnc = function (val, b) {
      b.value = val
    }
  ...
  window.setTimeout(function () {
    switch (destPageName) {
      ...
      case window.CONST.qIDEXIT:
        window.dialogAlert(
          "Author note: User's INCOMPLETE data would upload to server."
        )
        break
      ...
      case window.CONST.qIDFAIL:
        ...
        window.dialogAlert(
          'Author note: User would be redirected to another page: <a target=_blank href="' +
            url +
            '">' +
            url +
            '</a>'
      ...
      default:
        ...
        if (page === null || typeof page === 'undefined') {
          ...
          window.A2JViewer.layoutPage(
            window.$('.A2JViewer', '#page-viewer'),
            window.gPage
          )
          window
            .$('#page-viewer')
...
function getSelectedPageName () {
  // Return currently selected page name or '' if none selected.
  var rel = window.makestr(
    window
      .$('.pageoutline a.' + window.SELECTED)
      .first()
      .attr('rel')
  )
...
function createNewPage (newStep, mapx, mapy) {
  ...
  const newPageName =
    firedFromPagesTab && selectedPageName ? selectedPageName : 'New Page'
...
function pageRename (page, newName) {
  ...
  if (page.name.toLowerCase() !== newName.toLowerCase()) {
    if (window.gGuide.pages[newName]) {
      window.dialogAlert({
        title: 'Page rename disallowed',
        body: 'There is already a page named ' + newName
      })
      return false
    }
  }
...
function pageEditDelete (name) {
  ...
  window.dialogConfirmYesNo({
    ...
    message:
      '<div class="alert alert-danger"><span class="glyphicon-attention" aria-hidden="true"></span> Permanently delete page "' +
      name +
      '"?</div><div>' +
      txt +
      '</div>',
...
function gotoPageEdit (pageName) {
  ...
  var $qdeParentDiv = window
    .$('.page-edit-form-panel', $pageEditDialog)
    .html('')
...
function addDialogOverlay ($parentDialog) {
  var $dialogOverlay = window.$(
    '<div class="dialog-overlay ui-widget-overlay ui-front" />'
  )
...
function convertOptionsToText (optionsHTML) {
  ...
  if (optionsHTML) {
    ...
    convertedList = [].map
      .call(listSelect.children, function (option) {
...
function buildPageFieldSet (page) {
  ...
  if (page.type !== 'A2J') {
    pageFieldSet.append(
      window.form.h2('Page type/style: ' + page.type + '/' + page.style)
    )
  }
  pageFieldSet.append(
    window.form.htmlarea({
...
function buildLearnMoreFieldSet (page) {
  ...
  learnMoreFieldSet.append(
    window.form.htmlarea({
      label: 'Citation:',
      value: page.helpCitation,
      change: function (val) {
        page.helpCitation = val
      }
    })
  )
...
function buildFieldsFieldSet (page) {
  ...
  var fieldsListManager = window.form.listManager({
    name: 'Fields',
    ...
    create: function (ff, field) {
        ...
        window.form.pickList(
          {
            ...
            change: function (val, field, ff) {
              ...
              if (
                field.type === 'radio' ||
                field.type === window.CONST.ftCheckBoxNOTA
              ) {
                field.required = true
                ff.find('[name="required"]')
                  .find('[type=checkbox]')
                  .prop('checked', true)
              }
          ...
          [
            window.CONST.ftText,
            'Text',
            window.CONST.ftTextLong,
            'Text (Long)',
            window.CONST.ftTextPick,
            'Text (Pick from list)',
            window.CONST.ftNumber,
            'Number',
            window.CONST.ftNumberDollar,
            'Number Dollar',
            window.CONST.ftNumberSSN,
            'Number SSN',
            window.CONST.ftNumberPhone,
            'Number Phone',
            window.CONST.ftNumberZIP,
            'Number ZIP Code',
            window.CONST.ftNumberPick,
            'Number (Pick from list)',
            window.CONST.ftDateMDY,
            'Date MM/DD/YYYY',
            window.CONST.ftGender,
            'Gender',
            window.CONST.ftRadioButton,
            'Radio Button',
            window.CONST.ftCheckBox,
            'Check box',
            window.CONST.ftCheckBoxNOTA,
            'Check Box (None of the Above)',
            window.CONST.ftUserAvatar,
            'User Avatar'
          ]
        )
      )
      ...
      ff.append(
        window.form.text({
          ...
          name: 'maxchars',
          placeholder:
            "Enter a number here to set a Character Limit for the End User's response",
      ...
      var listText = field.previousTextList
        ? field.previousTextList
        : convertOptionsToText(field.listData)
...
var updateFieldLayout = function (ff, field) {
  ...
  var canRequire =
    field.type !== 'radio' &&
    field.type !== window.CONST.ftCheckBoxNOTA &&
    field.type !== window.CONST.ftUserAvatar
  var canMinMax =
    field.type === window.CONST.ftNumber ||
    field.type === window.CONST.ftNumberDollar ||
    field.type === window.CONST.ftNumberPick ||
    field.type === window.CONST.ftDateMDY
  var canList = field.type === window.CONST.ftTextPick
  var canDefaultValue =
    field.type !== window.CONST.ftCheckBox &&
    field.type !== window.CONST.ftCheckBoxNOTA &&
    field.type !== window.CONST.ftGender &&
    field.type !== window.CONST.ftUserAvatar
  var canOrder =
    field.type === window.CONST.ftTextPick ||
    field.type === window.CONST.ftNumberPick ||
    field.type === window.CONST.ftDateMDY
  var canUseCalc =
    field.type === window.CONST.ftNumber ||
    field.type === window.CONST.ftNumberDollar
  var canMaxChars =
    field.type === window.CONST.ftText ||
    field.type === window.CONST.ftTextLong ||
    field.type === window.CONST.ftNumberPhone ||
    field.type === window.CONST.ftNumberZIP ||
    field.type === window.CONST.ftNumberSSN
    ...
    window.form.listManager({
      ...
      create: function (ff, b) {
        ...
        var repeatOptions = [
          '',
          'Normal',
          window.CONST.RepeatVarSetOne,
          'Set Counting Variable to 1',
          window.CONST.RepeatVarSetPlusOne,
...
function buildLogicFieldSet (page) {
  ...
  logicfs.append(
    window.form.htmlarea({
      label: 'Logic Citation:',
      value: page.codeCitation,
      change: function (val) {
        page.codeCitation = val
      }
    })
  )
...
function guidePageEditForm (page, $qdeParentDiv) {
  ...
  if (window.CONST.showXML) {
    $qdeParentDiv.append(
      '<div class=xml>' + window.escapeHtml(page.xml) + '</div>'
    )
    $qdeParentDiv.append(
      '<div class=xml>' + window.escapeHtml(page.xmla2j) + '</div>'
    )
  }
...
window.TPage.prototype.tagList = function () {
  ...
  for (var f in page.fields) {
    ...
    if (field.required) {
      tags +=
        '<span class="label label-info tag"><span class="glyphicon-pencil" aria-hidden="true"></span>' +
        field.fieldTypeToTagName() +
  ...
  if (page.help !== '') {
    tags +=
      '<span class="label label-warning tag"><span class="glyphicon-lifebuoy" aria-hidden="true"></span>Help</span>'
  }
  if (page.codeAfter !== '' || page.codeBefore !== '') {
    tags +=
      '<span class="label label-success tag"><span class="glyphicon-split" aria-hidden="true"></span>Logic</span>'
  }
  if (page.repeatVar !== '') {
    tags +=
      '<span class="label label-danger glyphicon-cw" aria-hidden="true">Loop</span>'
  }
  if (page.outerLoopVar !== '') {
    tags +=
      '<span class="label label-danger glyphicon-cw" aria-hidden="true">Nested Loop</span>'
  }
  return tags
}
...
window.TGuide.prototype.pageFindReferences = function (findName, newName) {
  ...
  var testtext = function (page, field, fieldname) {
    ...
    page[field] = page[field].replace(window.REG.LINK_POP, function (
      match,
      p1,
      offset,
      string
  ...
  for (var p in guide.pages) {
    ...
    for (var bi in page.buttons) {
      ...
      if (b.next === findName) {
        ...
        if (newName != null) {
          // https://github.com/CCALI/CAJA/issues/2614
          b.next = newName
        }

        matches.push({
          name: page.name,
          field: 'Button ' + b.label,
          text: b.label,
          next: b.next
        })
      }
