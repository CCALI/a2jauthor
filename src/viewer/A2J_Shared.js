...
function pathExt (path) {	// Return extention file type
...
function isBlankOrNull (v) {	// Return true if v is blank or null.
...
function textToNumber (n) {	// Convert to number even with commas.
...
function aie (obj, attr, val) {	// set obj's attr to val, only if value exists
  if (val === null) {
    return
  }
  obj[attr] = val
...
function pickHilite (html, term) {	// find term in html text and hilite it
  return html.replace(
    new RegExp(
      '(?![^&;]+;)(?!<[^<>]*)(' +
...
*/
    var p
    for (p in o) {
      if (parseInt(p, 10) >= 0) {	// array assumed
...
function propsJSON_ (name, d, o) {	// 10/17/2012 Sam's simple JSON to XML.
  var body = ''
...
function isValidDate (d) {	// 06/02/2015 Test if JS date is valid
  // http://stackoverflow.com/questions/1353684/detecting-an-invalid-date-date-instance-in-javascript
  if (Object.prototype.toString.call(d) !== '[object Date]') { return false }
  return !isNaN(d.getTime())
}
function mdy2jsDate (MDY) {	//  2014-06-16 Convert a2j m/d/y date to JavaScript date object for use in calculations
  if (makestr(MDY) !== '') {
    var parts = MDY.split('/')
    if (parts[0] > 100) {
  ...
  } else {	// return today if we don't recognize it.
    var d = new Date()
    return new Date(d.getFullYear(), d.getMonth(), d.getDate())
  }
}
function ismdy (str) {	//  Return true if str looks like a date (m/d/y)
...
function days2jsDate (numDays) {	// Return JS Date based on days since 1/1/1970
  var d = new Date()
...
function jsDate2mdy (d) {	// 2014-06-16 Convert js date to A2J's M/D/Y format
  // trace('jsDate2mdy',d,(d.getMonth()+1)+'/'+d.getDate()+'/'+d.getFullYear());
  return (d.getMonth() + 1) + '/' + d.getDate() + '/' + d.getFullYear()
}
function jsDate2days (d) {	// Convert JS date into days since 1/1/1970
  // 2015-06-25 Git Issue #259 Ensure no time element.
  return Math.floor(d.getTime() / (1000 * 60 * 60 * 24))
}

function mdyTodmy (MDY) {	// 7/1/05 convert internal month/day/year format to HotDocs day/month/year format.
  var result = ''
  if (makestr(MDY) !== '') {
    var parts = MDY.split('/')
...
function dmyTomdy (DMY) {	// 12/22/2014 convert HotDocs day/month/year format internal month/day/year format.
  var result = ''
  if (makestr(DMY) !== '') {
    var parts = DMY.split('/')
    result = parts[1] + '/' + parts[0] + '/' + parts[2]
...
function scrollToElt (container, scrollTo) {	// Scroll container so that element scrollTo is visible.
...
function traceTag (cname, chtml) {	// cname is tag name, chtml is htmlized value
  if (cname === 'val') {
    if (chtml === '' || chtml === null) {
      chtml = 'blank'
      cname = 'valBlank'
...
/** @param {...} var_args */
function trace (var_args) {	// Send message to brower's console. Mainly for internal developers.
...
function audioPlayerHTML (audiofile) {	// 2014-06-03 Construct HTML 5 audio HTML. Currently only MP3 supported.
  return '<audio controls><source src="' + fixPath(audiofile) + '" type="audio/mpeg"></audio>'
}

function videoPlayerHTML (videoFile) {	// 2014-06-03 Construct HTML 5 video HTML. FLV no longer supported.
